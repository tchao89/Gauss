        ...
        if len(running_experiment_list) > 1:
            ...
            for key in running_experiment_list:
                experiment_information += EXPERIMENT_DETAIL_FORMAT % (key,
                                                                      experiments_dict[key].get('experimentName',
                                                                                                'N/A'),
                                                                      ...
                                                                      time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(
                                                                          experiments_dict[key][
                                                                              'startTime'] / 1000)) if isinstance(
                                                                          experiments_dict[key]['startTime'], int) else
                                                                      experiments_dict[key]['startTime'],
                                                                      time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(
                                                                          experiments_dict[key][
                                                                              'endTime'] / 1000)) if isinstance(
                                                                          experiments_dict[key]['endTime'], int) else
                                                                      experiments_dict[key]['endTime'])
...
def convert_time_stamp_to_date(content):
    ...
    if start_time_stamp:
        start_time = datetime.fromtimestamp(start_time_stamp // 1000, timezone.utc).astimezone().strftime(
            "%Y/%m/%d %H:%M:%S")
        content['startTime'] = str(start_time)
    if end_time_stamp:
        end_time = datetime.fromtimestamp(end_time_stamp // 1000, timezone.utc).astimezone().strftime(
            "%Y/%m/%d %H:%M:%S")
...
def webui_nas(args):
    ...
    try:
        ...
        cmds = [node_command, '--max-old-space-size=4096', entry_file, '--port', str(args.port), '--logdir',
                args.logdir]
...
def hdfs_clean(host, user_name, output_dir, experiment_id=None):
    '''clean up hdfs data'''
    hdfs_client = HdfsClient(hosts='{0}:80'.format(host), user_name=user_name, webhdfs_path='/webhdfs/api/v1',
                             timeout=5)
    if experiment_id:
...
def experiment_clean(args):
    ...
    while True:
        print('INFO: This action will delete experiment {0}, and it\'s not recoverable.'.format(
            ' '.join(experiment_id_list)))
    ...
    for experiment_id in experiment_id_list:
        experiment_id = get_config_filename(args)
        experiment_config = Config(experiment_id,
                                   Experiments().get_all_experiments()[experiment_id]['logDir']).get_config()
...
def show_experiment_info():
    ...
    for key in experiment_id_list:
        print(EXPERIMENT_MONITOR_INFO % (key, experiments_dict[key]['status'], experiments_dict[key]['port'], \
                                         experiments_dict[key].get('platform'), time.strftime('%Y-%m-%d %H:%M:%S',
                                                                                              time.localtime(
                                                                                                  experiments_dict[key][
                                                                                                      'startTime'] / 1000)) if isinstance(
            experiments_dict[key]['startTime'], int) else experiments_dict[key]['startTime'], \
                                         get_time_interval(experiments_dict[key]['startTime'],
                                                           experiments_dict[key]['endTime'])))
        ...
        if args.type == 'json':
            ...
            for record in content:
                ...
                if not isinstance(record_value, (float, int)):
                    formated_record.update(
                        {**record['parameter'], **record_value, **{'trialJobId': record['trialJobId']}})
                else:
                    formated_record.update(
                        {**record['parameter'], **{'reward': record_value, 'trialJobId': record['trialJobId']}})
