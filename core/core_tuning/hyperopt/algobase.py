...
class ExprEvaluator(object):
    def __init__(self, expr,
        ...
        if max_program_len is None:
            self.max_program_len = core.hyperopt.pyll.base.DEFAULT_MAX_PROGRAM_LEN
        else:
    ...
    def set_in_memo(self, memo, k, v):
        ...
        """
        if self.memo_gc:
            assert v is not core.hyperopt.pyll.base.GarbageCollected
            memo[k] = v
            for ii in k.inputs():
                ...
                if all(iic in memo for iic in self.clients[ii]):
                    memo[ii] = core.hyperopt.pyll.base.GarbageCollected
        else:
            memo[k] = v
    ...
    def on_node(self, memo, node):
        ...
        if self.memo_gc:
            ...
            for aa in args + list(kwargs.values()):
                assert aa is not core.hyperopt.pyll.base.GarbageCollected
