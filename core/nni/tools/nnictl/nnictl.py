...
def parse_args():
    ...
    # NAS algorithm for search space generation
    parser_start = subparsers.add_parser('ss_gen', help='automatically generate search space file from trial code')
    parser_start.add_argument('--trial_command', '-t', required=True, dest='trial_command',
                              help='the command for running trial code')
    parser_start.add_argument('--trial_dir', '-d', default='./', dest='trial_dir',
                              help='the directory for running the command')
    parser_start.add_argument('--file', '-f', default='nni_auto_gen_search_space.json', dest='file',
                              help='the path of search space file')
    ...
    parser_start.add_argument('--port', '-p', default=DEFAULT_REST_PORT, dest='port', type=int,
                              help='the port of restful server')
    parser_start.add_argument('--debug', '-d', action='store_true', help=' set debug mode')
    parser_start.add_argument('--foreground', '-f', action='store_true',
                              help=' set foreground mode, print log content to terminal')
    parser_start.set_defaults(func=create_experiment)

    # parse resume command
    # resume last experiment
    ...
    parser_resume.add_argument('--port', '-p', default=DEFAULT_REST_PORT, dest='port', type=int,
                               help='the port of restful server')
    parser_resume.add_argument('--debug', '-d', action='store_true', help=' set debug mode')
    parser_resume.add_argument('--foreground', '-f', action='store_true',
                               help=' set foreground mode, print log content to terminal')
    ...
    parser_view.add_argument('--port', '-p', default=DEFAULT_REST_PORT, dest='port', type=int,
                             help='the port of restful server')
    ...
    parser_updater_duration.add_argument('--value', '-v', required=True,
                                         help='the unit of time should in {\'s\', \'m\', \'h\', \'d\'}')
    ...
    parser_trial_kill.add_argument('--trial_id', '-T', required=True, dest='trial_id',
                                   help='the id of trial to be killed')
    parser_trial_kill.set_defaults(func=trial_kill)
    parser_trial_codegen = parser_trial_subparsers.add_parser('codegen',
                                                              help='generate trial code for a specific trial')
    parser_trial_codegen.add_argument('id', nargs='?', help='the id of experiment')
    parser_trial_codegen.add_argument('--trial_id', '-T', required=True, dest='trial_id',
                                      help='the id of trial to do code generation')
    ...
    # export trial data
    parser_trial_export = parser_experiment_subparsers.add_parser('export',
                                                                  help='export trial job results to csv or json')
    parser_trial_export.add_argument('id', nargs='?', help='the id of experiment')
    parser_trial_export.add_argument('--type', '-t', choices=['json', 'csv'], required=True, dest='type',
    ...
    parser_load_experiment.add_argument('--logDir', '-l', required=False,
                                        help='the path of logDir for loaded experiment')
    ...
    parser_platform_clean.add_argument('--config', '-c', required=True, dest='config',
    ...
    parser_package_subparsers = subparsers.add_parser('package', help='this argument is replaced by algo',
                                                      prefix_chars='\n')
    ...
    parser_tensorboard_start.add_argument('--port', dest='port', default=6006, type=int,
                                          help='the port to start tensorboard')
    ...
    parser_top.add_argument('--time', '-t', dest='time', type=int, default=3,
                            help='the time interval to update the experiment status, ' \
                                 'the unit is second')
