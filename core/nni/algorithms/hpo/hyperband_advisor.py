...
class Bracket:
    ...
    eta: float
        In each iteration, a complete run of sequential halving is executed. In it, after evaluating each configuration on the same subset size, only a fraction of 1/eta of them 'advances' to the next round.
    R:
    ...
    def _get_one_trial_job(self):
        ...
        if not self.generated_hyper_configs:
            if self.exec_mode == 'parallelism' or \
                    (self.exec_mode == 'serial' and (
                            self.curr_bracket_id is None or self.brackets[self.curr_bracket_id].is_completed())):
                if self.curr_s < 0:
                    self.curr_s = self.s_max
                    self.curr_hb += 1
                ...
                self.brackets[self.curr_bracket_id] = Bracket(self.curr_bracket_id, self.curr_s, self.s_max, self.eta,
                ...
                generated_hyper_configs = self.brackets[self.curr_bracket_id].get_hyperparameter_configurations(next_n,
                                                                                                                next_r,
