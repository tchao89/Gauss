...
class TFPlainDataset(BaseDataset):
    ...
    def __init__(self, **params):
        ...
        if not isinstance(params["target_name"], List):
            raise TypeError("argument target_name must be a list, got <{arg0}> instead."\
                .format(arg0=str(type(params["target_name"])).split("'")[1]))
        ...
        self._df_dataset = self._h_stack(params["dataset"])
    ...
    def update_dataset_parameters(self, **params):
        ...
        if not hasattr(self, "_next_batch"):
            raise AttributeError("dataset has not been initialized to batches.")
    ...
    def _filter_feature(self, dataset: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        dataset = dataset[self._selected_features]
        return dataset
